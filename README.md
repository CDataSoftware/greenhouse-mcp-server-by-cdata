...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Greenhouse](https://www.cdata.com/drivers/greenhouse/download/mcp/). 
## Purpose
...
CData JDBC Driver connects to Greenhouse by exposing them as relational SQL models.

This server wraps that driver and makes Greenhouse data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.

## Setup Guide
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/greenhouse-mcp-server-by-cdata.git
      cd greenhouse-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/greenhouse/download/jdbc](https://www.cdata.com/drivers/greenhouse/download/jdbc/)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Greenhouse\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Greenhouse/`
    * Run the command `java -jar cdata.jdbc.greenhouse.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.greenhouse.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `greenhouse.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.greenhouse.GreenhouseDriver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=greenhouse
      ServerName=CDataGreenhouse
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.greenhouse.jar
      DriverClass=cdata.jdbc.greenhouse.GreenhouseDriver
      JdbcUrl=jdbc:greenhouse:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
